public class WordsCounterImplement implements WordsCounter {
		...
		for (int i = 0; i < words.length; i++)// for
		{
			...
			if (i == 0) {// if 0
				...
				for (int k = 0; k < words.length; k++) {
					...
					if (StringCompare.equals(buf, words[k])) {
						countWords++;
					}
				}// for
			...
			if (i != 0) {// if2
				...
				for (int j = 0; j < word.length(); j++)// for1
				{
					...
					if (StringCompare.equals(buf, ((String) word.getVal(j)))) {// if
																				// 1
						inWord = true;
				...
				if (inWord == false) {// if3
					...
					for (int z = 0; z < words.length; z++) {// for2
						...
						if (StringCompare.equals(buf, words[z])) {
							countWords++;
						}
